...
exports.register = function (plugin, options, pluginNext) {
    ...
    var jsBaseUrl = (settings.endpoint === '/' ? '' : settings.endpoint) + '/js';
        ...
        server.views({
            ...
            runtimeOptions: {
                data: {
                    ...
                    jsBaseUrl: jsBaseUrl.replace(/(.*?)((\/\w+)?\/css)/, '$2')
                }
            }
        });
        ...
        handler: function (request, reply) {
            ...
            routingTable.forEach(function (connection) {
                ...
                connection.table = connection.table.filter(function (item) {
                    ...
                    /* skip when no matches */
                    if ((request.query.path && item.path !== request.query.path) || (request.query.method && item.method && item.method !== request.query.method.toLowerCase())) {
                        return false;
                    }
...
internals.getRoutesData = function (routes) {
    ...
    return routes.map(function (route) {
        ...
        return {
            ...
            cors: route.settings.cors && Object.keys(route.settings.cors)
                .filter((key) => key[0] != '_')
                .reduce((acc,k) => (acc[k] = route.settings.cors[k], acc), {}),
